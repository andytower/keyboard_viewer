...
class GuiManager():
    def __init__(self,master,config,queue,keyTrainer):
        ...
        self.buttonFont=font.Font(family=config.font_name,size=config.font_size)
        self.boldUnderscoredButtonFont=font.Font(family=config.font_name,size=config.font_size,weight='bold',underline=1)

        for row_index in range(1,config.getNumOfRows()+1):
            self.gui_rows[int(row_index)]=Frame(master)
            self.gui_row_buttons[int(row_index)]=[]
            for button_num in range(1,config.getNumOfKeysInRow(row_index)+1):
